    ...
    try {
        ...
        const song = response.hits.find((hit) => {
            if (hit.type === 'song') {
                ...
                return hit.result.primary_artist.name.toLowerCase().search(artist) || artist.search(hit.result.primary_artist.name.toLowerCase()) || hit.result.primary_artist.name.toLowerCase() === artist;
            }
            return false;
        })
        if (!song) {
